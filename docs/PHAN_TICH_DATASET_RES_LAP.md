# Phân tích dataset Restaurant, Laptop và Restaurant tiếng Việt
...
- `dataset/rest_vi/train.multiple.json`
- `dataset/rest_vi/test.multiple.json`
...
| Rest VI | Train | 1.980 | 3.608 | 1,82 | 971 (49,04%) |
| Rest VI | Test | 600 | 1.120 | 1,87 | 315 (52,50%) |
...
Rest VI có cùng quy mô với Restaurant gốc vì đây là bản dịch tiếng Việt của cùng bộ dữ liệu.
...
| Rest VI | 807 (22,37%) | 637 (17,66%) | 2.164 (59,98%) | 3.608 |
...
- Rest VI: `positive / neutral = 3,40`.

Do đó Accuracy có nguy cơ đánh giá quá lạc quan trên Restaurant và Rest VI nhiều hơn Laptop.
...
| Rest VI | 196 (17,50%) | 196 (17,50%) | 728 (65,00%) | 1.120 |
...
| Rest VI | Train | 20,30 | 19 | 40 | 79 |
| Rest VI | Test | 19,40 | 18 | 40 | 70 |
...
| Rest VI | Train | 1.224 | 2,95 |
| Rest VI | Test | 524 | 2,14 |
...
| Restaurant | Số mẫu | Laptop | Số mẫu | Rest VI | Số mẫu |
|---|---:|---|---:|---|---:|
| food | 361 | screen | 61 | đồ ăn | 226 |
| service | 225 | price | 56 | dịch vụ | 222 |
| prices | 63 | use | 54 | giá | 92 |
| place | 59 | battery life | 53 | món ăn | 87 |
| menu | 56 | keyboard | 50 | địa điểm | 64 |
| dinner | 56 | battery | 47 | thức ăn | 62 |
| staff | 55 | programs | 37 | nhân viên | 60 |
| pizza | 51 | features | 35 | pizza | 54 |
| atmosphere | 46 | software | 33 | không khí | 53 |
| price | 41 | warranty | 32 | bàn | 53 |
...
Rest VI giữ lại đúng cấu trúc này nhưng với nhãn aspect tiếng Việt.
...
| Rest VI | 45 (12,43%) | 91 (25,14%) | 226 (62,43%) |

Nhóm unseen Laptop không chỉ lớn hơn mà còn cân bằng hơn, chứa nhiều neutral hơn. Đây là nhóm khó, ít được hưởng lợi từ bias positive và có thể giải thích một phần chênh lệch Accuracy giữa hai miền.

Lưu ý: notebook có thể báo tỷ lệ unseen khác giữa các validation split vì tập tham chiếu là phần train của từng split. Con số 31,52%, 42,01% và 32,32% ở đây dùng toàn bộ train sạch làm tập tham chiếu và cùng cách so khớp aspect của pipeline.
...
- Tập nhỏ hơn nhưng vocabulary aspect rộng, làm số ví dụ trên mỗi unique aspect thấp.
...
- Là bản dịch tiếng Việt của Restaurant, nên giữ nguyên cấu trúc mất cân bằng positive.
- Aspect tiếng Việt giúp kiểm tra pipeline multilingual thay vì chỉ bám vào dữ liệu tiếng Anh.
- Tỷ lệ unseen aspect và phân phối nhãn gần với Restaurant gốc, nhưng có thêm rủi ro do dịch máy và chuẩn hóa từ vựng.

## 8. Hàm ý cho mô hình và thí nghiệm

- Dùng Macro-F1 làm metric chính; không chọn mô hình chỉ theo Accuracy.
- Giữ split theo câu vì tỷ lệ câu nhiều aspect cao ở cả ba dataset.
...
- Khi so sánh giữa miền, giữ nguyên pipeline nhưng không giả định hyperparameter tối ưu của Restaurant sẽ chuyển trực tiếp sang Laptop hoặc Rest VI.
...
Rest VI là bản tiếng Việt của Restaurant nên giữ gần như toàn bộ đặc tính dữ liệu của miền gốc, đồng thời thêm rủi ro dịch máy và chuẩn hóa aspect tiếng Việt.

Các đặc tính này phù hợp với kết quả mô hình: Laptop có Accuracy và Macro-F1 thấp hơn, calibration kém hơn và nhạy seed hơn. Ưu tiên cải thiện cho Laptop và Rest VI nên là biểu diễn unseen aspect, chuẩn hóa aspect và tăng độ ổn định qua nhiều seed.
